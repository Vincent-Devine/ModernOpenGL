...
namespace Resources
{
	...
	public:
		...
		template <typename T>
		T* Create(const std::string p_name);
	...
	template <typename T>
	T* ResourceManager::Create(const std::string p_name)
	{
		static_assert(std::is_base_of<IResource, T>::value, "T is not a compatible resource");
		
		const unsigned int id = resources.size();
		auto [it, _] = resources.insert_or_assign(p_name, std::make_unique<T>(p_name, id));
