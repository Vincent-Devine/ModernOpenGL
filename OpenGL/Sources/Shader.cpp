...
namespace Resources
{
	Shader::Shader(const std::string& p_name, const unsigned int p_id)
		: vertexShader(0)
		, fragmentShader(0)
		, shaderProgram(0)
	{
	...
	void Shader::Init(const std::string& p_pathVert, const std::string& p_pathFrag)
	{
		// build and compile our shader program
		Assertion(SetVertexShader(p_pathVert), "Fail on SetVertexShader()");
		Assertion(SetFragmentShader(p_pathFrag), "Fail on SetFragmentShader()");
	...
	bool Shader::SetVertexShader(const std::string& p_path)
	{
		std::ifstream sourcefile;
		std::string tempSource;
		std::string line;
		OpenShader(p_path, sourcefile);
		while (std::getline(sourcefile, line))
			tempSource += "\n" + line;
		CloseShader(p_path, sourcefile);
	...
	bool Shader::SetFragmentShader(const std::string& p_path)
	{
		std::ifstream sourcefile;
		std::string tempSource;
		std::string line;
